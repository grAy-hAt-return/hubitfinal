...
function OverviewCourse() {
  ...
  return (
    <div className='w-full px-2'>
        <div className='w-full grid grid-cols-3 my-10 rounded-sm overflow-hidden'>
            {data.map((val,i)=>{
                return <div key={i} className={` ${val.color} w-full h-32 flex flex-col items-center justify-center text-white ${val.text ? "text-black":"text-white"}`}>
                    <h4>{val.h1}</h4>
                    <p>{val.detail}</p>
                </div>
            })}
